...
After your EHRSHOT access is approved, sign in to Redivis and create an API
token by following the [Redivis API authentication instructions](https://docs.redivis.com/api/client-libraries/redivis-python/getting-started).
Export the token before running the downloader:

```bash
export REDIVIS_API_TOKEN="your-token"
```

Treat the token like a password and do not commit it to the repository.

## Supported Tasks

No MEDS-DEV task currently lists EHRShot as a supported dataset.

## Predicates

No EHRShot predicates are included in this initial dataset integration.
